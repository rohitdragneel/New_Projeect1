...
def verify():
    try:
        CREATE_TABLE = """CREATE TABLE IF NOT EXISTS signcout (id TEXT PRIMARY KEY)"""
        cursor = connect_to_db().cursor()
        cursor.execute(CREATE_TABLE)
        cursor.connection.commit()
        cid = """SELECT id FROM signcout ORDER BY id DESC """
        ...
        if item :
            ...
            if item1==None :
                ...
                conn = connect_to_db()
                cursor = conn.cursor()
                query = """INSERT INTO signcout VALUES(?)"""
        ...
        if dist <= 0.145139:  # Threshold obtained using Test.py
            return jsonify({"match": True, "error": False, "threshold": "%.6f" % (0.145139), "distance": "%.6f" % (mindist),"difference":str(out_arr),'fileid':mainid})
        return jsonify({"match": False, "error": False, "threshold": 0.145139, "distance": round(mindist, 6),"difference":str(out_arr),'fileid':mainid})
    except Exception as e:
        print(e)
